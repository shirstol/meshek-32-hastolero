...
public class OrderService {
 private final CustomerRepository customers; private final ProductRepository products; private final DistributionPointRepository distributionPoints; private final CustomerOrderRepository orders; private final StoreProductRepository storeProducts;
 public OrderService(CustomerRepository customers, ProductRepository products, DistributionPointRepository distributionPoints, CustomerOrderRepository orders, StoreProductRepository storeProducts) { this.customers=customers; this.products=products; this.distributionPoints=distributionPoints; this.orders=orders; this.storeProducts=storeProducts; }
 ...
 @Transactional public OrderResponse createOrder(CreateOrderRequest request) {
  ...
  List<StoreProduct> selected=storeProducts.findByStoreIdAndProductIdIn(point.getId(),requested.keySet()); if(selected.size()!=requested.size()) throw badRequest("אחד המוצרים אינו זמין בחנות הזאת"); BigDecimal total=BigDecimal.ZERO;
  for(StoreProduct offer:selected){int quantity=requested.get(offer.getProduct().getId()); if(!offer.isAvailable()) throw badRequest(offer.getProduct().getName()+" אינו זמין"); if(offer.getMaxQuantity()!=null&&quantity>offer.getMaxQuantity()) throw badRequest("חרגת ממגבלת הכמות של "+offer.getProduct().getName()); total=total.add(offer.getPrice().multiply(BigDecimal.valueOf(quantity)));}
  CustomerOrder order=new CustomerOrder("M32-"+UUID.randomUUID().toString().substring(0,8).toUpperCase(),customer,point,total,request.paymentMethod()); selected.forEach(offer->order.addItem(offer.getProduct(),offer.getPrice(),requested.get(offer.getProduct().getId()))); return orderResponse(orders.save(order));
 }
 ...
 public static ProductResponse productResponse(Product product){return new ProductResponse(product.getId(),product.getName(),product.getDescription(),product.getCategory(),product.getImageUrl(),product.getPrice(),product.isAvailable(),product.getMaxQuantity());}
 public static ProductResponse offerResponse(StoreProduct offer){Product product=offer.getProduct();return new ProductResponse(product.getId(),product.getName(),product.getDescription(),product.getCategory(),product.getImageUrl(),offer.getPrice(),offer.isAvailable(),offer.getMaxQuantity());}
 public StoreCatalogResponse storeCatalog(String slug){DistributionPoint store=distributionPoints.findBySlugAndActiveTrue(slug).orElseThrow(()->new ResponseStatusException(HttpStatus.NOT_FOUND,"החנות לא נמצאה"));StoreResponse response=new StoreResponse(store.getId(),store.getSlug(),store.getLocality(),store.getName(),store.getDetails(),store.getAboutText());return new StoreCatalogResponse(response,storeProducts.findByStore_SlugOrderByProduct_NameAsc(slug).stream().map(OrderService::offerResponse).toList());}
 @Transactional public ProductResponse addProductToStore(String slug,AdminStoreProductRequest request){DistributionPoint store=distributionPoints.findBySlugAndActiveTrue(slug).orElseThrow(()->new ResponseStatusException(HttpStatus.NOT_FOUND,"החנות לא נמצאה"));String id="product-"+UUID.randomUUID().toString();Product product=products.save(new Product(id,request.name().trim(),request.description().trim(),request.category().trim(),blankToNull(request.imageUrl()),request.price(),request.available(),request.maxQuantity()));StoreProduct offer=storeProducts.save(new StoreProduct(store,product,request.price(),request.available(),request.maxQuantity()));return offerResponse(offer);}
 @Transactional public ProductResponse updateProductInStore(String slug,String productId,AdminStoreProductRequest request){StoreProduct offer=findOffer(slug,productId);offer.getProduct().updateCatalogDetails(request.name().trim(),request.description().trim(),request.category().trim(),blankToNull(request.imageUrl()));offer.updateOffer(request.price(),request.available(),request.maxQuantity());return offerResponse(offer);}
 @Transactional public void removeProductFromStore(String slug,String productId){storeProducts.delete(findOffer(slug,productId));}
 ...
 private StoreProduct findOffer(String slug,String productId){return storeProducts.findByStore_SlugAndProduct_Id(slug,productId).orElseThrow(()->new ResponseStatusException(HttpStatus.NOT_FOUND,"המוצר לא נמצא בחנות"));}
