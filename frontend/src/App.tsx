...
function App() {
  ...
  async function submitGuest(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setSubmitting(true); setError(''); try { await api.identifyCustomer(checkout.fullName, checkout.phone); setScreen('catalog') } catch { setError('לא ניתן לשמור את פרטי הלקוח. ודאי שהשרת פעיל ונסי שוב.') } finally { setSubmitting(false) } }
